...
class BankPersistenceSpockTest extends SpockPersistenceTestAbstractClass {
    ...
    def thenAssert() {
        ...
        def banks = new ArrayList<Bank>(FenixFramework.getDomainRoot().getBankSet())
        def bank = banks.get(0)

        assert BANK_NAME == bank.getName()
        assert BANK_CODE == bank.getCode()
        assert bank.getClientSet().size() == 1
        assert bank.getAccountSet().size()
        assert bank.getOperationSet().size() == 1

        def clients = new ArrayList<Client>(bank.getClientSet())
        def client = clients.get(0)

        assert client.getName() == CLIENT_NAME

        def accounts = new ArrayList<Account>(bank.getAccountSet())
        ...
        def operations = new ArrayList<Operation>(bank.getOperationSet())
